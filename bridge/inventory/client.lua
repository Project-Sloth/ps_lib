...
for _, v in ipairs(options) do
    if GetResourceState(v.name) == 'started' then
        local filePath = ('bridge/inventory/%s/client/%s.lua'):format(v.module, v.module)
...
AddEventHandler('onResourceStart', function(resourceName)
    if fallback[resourceName] then
        local filePath = ('bridge/inventory/%s/client/%s.lua'):format(fallback[resourceName], fallback[resourceName])
        local content = loadLib(filePath)
        if content then
