...
---@return Array
function ps.array:from(iter)
    local iterType = type(iter)

    if iterType == 'table' then
        return ps.array:new(table_unpack(iter))
    end

    if iterType == 'string' then
        return ps.array:new(string.strsplit('', iter))
    end

    if iterType == 'function' then
        local arr = ps.array:new()
        local length = 0
...
function ps.array:fill(value, start, endIndex)
    local length = #self
    start = start or 1
...
---@param testFn fun(element: unknown): boolean
function ps.array:filter(testFn)
    local newArr = {}
    local length = 0
...
---@param testFn fun(element: unknown): boolean
---@param last? boolean
function ps.array:find(testFn, last)
...
---@param testFn fun(element: unknown): boolean
---@param last? boolean
function ps.array:findIndex(testFn, last)
    local a = last and #self or 1
    local b = last and 1 or #self
    local c = last and -1 or 1
...
---@param value unknown
---@param last? boolean
function ps.array:indexOf(value, last)
    local a = last and #self or 1
    local b = last and 1 or #self
    local c = last and -1 or 1
...
function ps.array:includes(element, fromIndex)
...
function ps.array:join(seperator)
...
---@return T
function ps.array:reduce(reducer, initialValue, reverse)
    local length = #self
...
---@param start? number
---@param finish? number
function ps.array:slice(start, finish)
    local length = #self
    start = start or 1
...
function ps.array:toReversed()
    local reversed = ps.array:new()
...
---@param tbl ArrayLike
---@return boolean
function ps.array.isArray(tbl)
    local tableType = table_type(tbl)

    if not tableType then return false end

    if tableType == 'array' or tableType == 'empty' or ps.array.instanceOf(tbl, ps.array) then
        return true
    end
